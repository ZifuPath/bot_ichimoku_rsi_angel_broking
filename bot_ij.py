...
STOCKS = {'TATAMOTORS': 3456, 'JSWSTEEL':11723 , 'AXISBANK': 5900, 'RELIANCE': 2885, 'DRREDDY': 881,
    ...
    while dt.time(9, 20) < datetime.now().time() < dt.time(15, 14):
        ...
        try:
            ...
            if all(conditions_buy):
                ...
                TYPE = 'BUY'
                QUANTITY=round(20000*4/buy_price)
                quantity=round(500/abs(buy_price - stop_loss))
            ...
            if all(conditions_sell):
                ...
                TYPE = 'SELL'
                QUANTITY = round(20000 * 2 / sell_price)
                quantity = round(500 / abs(sell_price - stop_loss))
                if QUANTITY > quantity:
                    QUANTITY = quantity
                ltpdata = obj.ltpData(exchange='NSE', tradingsymbol=tradingsymbol, symboltoken=token)
                ltp = ltpdata['data']['ltp']
...
if __name__ == '__main__':
    ...
    task1 = multiprocessing.Process(target=bot_ij, args=('SAIL', obj, 0.85 / 100, 0.85 / 100)).start()
    # task2 = multiprocessing.Process(target=bot_ij, args=('', obj, 0.85 / 100, 0.85 / 100)).start()